...
const AllMovies = () => {
  ...
  useEffect(()=>{
    fetch(`http://localhost:5000/movies/${activeTab}`)
    .then(res => res.json())
    .then(data=>{
        setMovies(data);
    })
...
const handleViewMovie = (id)=>{
  ...
  return (
    ...
    <div className="grid grid-cols-4">
                {
                    movies?.map(movie=> <div onClick={()=>handleViewMovie(movie._id)} key={movie._id} className="card ">
                    <figure><Link to={`/view-details/${movie._id}`}><img className="h-[330px] w-[240px]" src={movie.img} alt="movie" /></Link></figure>
